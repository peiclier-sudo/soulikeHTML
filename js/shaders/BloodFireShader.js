 ...
 *
 * Two quality tiers:
 *   FULL  – 10 FBM calls, domain warping, vertex displacement  (GPU-heavy)
 *   FAST  – 0 FBM, sin/cos approximation, same palette          (20-50x cheaper)
 */
...
const fastFragmentShader = `
    ...
    void main() {
        ...
        // Cheap plasma: 4 sin/cos instead of 30 noise lookups
        float n1 = sin(p.x * 2.0 + t * 0.4) * cos(p.z * 2.5 + t * 0.3) * 0.5 + 0.5;
        float n2 = sin(p.y * 3.0 + t * 0.6 + 1.7) * cos(p.x * 1.8 - t * 0.5) * 0.5 + 0.5;
        float plasma = n1 * 0.6 + n2 * 0.4;
        plasma = smoothstep(0.2, 0.8, plasma);

        float veins = smoothstep(0.6, 0.8, n1) * smoothstep(0.5, 0.7, n2);
        float embers = smoothstep(0.85, 0.95, sin(p.x * 8.0 + t * 4.0) * cos(p.z * 7.0 - t * 3.0) * 0.5 + 0.5);
...
export function setBloodFireQuality(fast) {
